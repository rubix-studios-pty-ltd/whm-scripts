...
TMP_DIR="$(mktemp -d)"
COMMENT_PREFIX="# managed-by=rubix-imunify-rankmath"
...
run_source "rankmath" fetch_static_prefixes \
  "rankmath" \
  "172.66.40.202" \
  "172.66.43.54" \
  "193.138.6.5" \
...
if [ "$FAILURES" -gt 0 ]; then
  echo "Rank Math whitelist sync complete with $FAILURES failed source(s)."
  exit 1
fi

echo "Rank Math whitelist sync complete."
