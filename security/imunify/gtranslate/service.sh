...
TMP_DIR="$(mktemp -d)"
COMMENT_PREFIX="# managed-by=rubix-imunify-gtranslate"
...
run_source "gtranslate" fetch_plain_list \
  "gtranslate" \
...
if [ "$FAILURES" -gt 0 ]; then
  echo "GTranslate whitelist sync complete with $FAILURES failed source(s)."
  exit 1
fi

echo "GTranslate whitelist sync complete."