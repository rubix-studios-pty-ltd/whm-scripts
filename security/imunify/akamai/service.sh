...
TMP_DIR="$(mktemp -d)"
COMMENT_PREFIX="# managed-by=rubix-imunify-akamai"
...
fetch_whitespace_cidr_lists() {
  ...
  {
    echo "$COMMENT_PREFIX service=$name updated=$DATE source=akamai"
    sort -u "$tmp_ips"
  } > "$tmp_out"
...
run_source "akamai" fetch_whitespace_cidr_lists \
  "akamai" \
...
if [ "$FAILURES" -gt 0 ]; then
  echo "Akamai whitelist sync complete with $FAILURES failed source(s)."
  exit 1
fi

echo "Akamai whitelist sync complete."
