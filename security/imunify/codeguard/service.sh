...
TMP_DIR="$(mktemp -d)"
COMMENT_PREFIX="# managed-by=rubix-imunify-codeguard"
...
run_source "codeguard" fetch_static_prefixes \
  "codeguard" \
  "54.236.233.46" \
  "54.236.233.28" \
  "54.174.91.34" \
...
if [ "$FAILURES" -gt 0 ]; then
  echo "CodeGuard whitelist sync complete with $FAILURES failed source(s)."
  exit 1
fi

echo "CodeGuard whitelist sync complete."