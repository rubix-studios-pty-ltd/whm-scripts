...
TMP_DIR="$(mktemp -d)"
COMMENT_PREFIX="# managed-by=rubix-imunify-transparentcdn"
...
run_source "transparentcdn" fetch_json_list \
  "transparentcdn" \
...
if [ "$FAILURES" -gt 0 ]; then
  echo "Transparent CDN whitelist sync complete with $FAILURES failed source(s)."
  exit 1
fi

echo "Transparent CDN whitelist sync complete."