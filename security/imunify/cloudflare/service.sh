...
TMP_DIR="$(mktemp -d)"
COMMENT_PREFIX="# managed-by=rubix-imunify-cloudflare"
...
fetch_plain_lists() {
  ...
  {
    echo "$COMMENT_PREFIX service=$name updated=$DATE source=cloudflare"
    sort -u "$tmp_ips"
  } > "$tmp_out"
...
run_source "cloudflare" fetch_plain_lists \
  "cloudflare" \
...
if [ "$FAILURES" -gt 0 ]; then
  echo "Cloudflare whitelist sync complete with $FAILURES failed source(s)."
  exit 1
fi

echo "Cloudflare whitelist sync complete."