...
TMP_DIR="$(mktemp -d)"
COMMENT_PREFIX="# managed-by=rubix-imunify-fastly"
...
install_list() {
  ...
  if ! is_valid_list "$tmp_out"; then
    echo "No valid IP addresses or CIDR ranges found for $name."
...
run_source "fastly" fetch_fastly_public_ip_list \
  "fastly" \
...
if [ "$FAILURES" -gt 0 ]; then
  echo "Fastly whitelist sync complete with $FAILURES failed source(s)."
  exit 1
fi

echo "Fastly whitelist sync complete."