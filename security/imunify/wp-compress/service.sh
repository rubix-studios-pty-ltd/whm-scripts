...
TMP_DIR="$(mktemp -d)"
COMMENT_PREFIX="# managed-by=rubix-imunify-wp-compress"
...
run_source "wp-compress" fetch_plain_list \
  "wp-compress" \
...
if [ "$FAILURES" -gt 0 ]; then
  echo "WP Compress whitelist sync complete with $FAILURES failed source(s)."
  exit 1
fi

echo "WP Compress whitelist sync complete."