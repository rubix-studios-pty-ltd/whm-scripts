...
TMP_DIR="$(mktemp -d)"
COMMENT_PREFIX="# managed-by=rubix-imunify-statuscake"
...
run_source "statuscake" fetch_statuscake_json \
  "statuscake" \
...
if [ "$FAILURES" -gt 0 ]; then
  echo "StatusCake whitelist sync complete with $FAILURES failed source(s)."
  exit 1
fi

echo "StatusCake whitelist sync complete."