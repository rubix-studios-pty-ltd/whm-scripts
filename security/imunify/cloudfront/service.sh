...
TMP_DIR="$(mktemp -d)"
COMMENT_PREFIX="# managed-by=rubix-imunify-cloudfront"
...
run_source "cloudfront" fetch_cloudfront_json \
  "cloudfront" \
...
if [ "$FAILURES" -gt 0 ]; then
  echo "CloudFront whitelist sync complete with $FAILURES failed source(s)."
  exit 1
fi

echo "CloudFront whitelist sync complete."