...
TMP_DIR="$(mktemp -d)"
COMMENT_PREFIX="# managed-by=rubix-imunify-sendcloud"
...
run_source "sendcloud" fetch_plain_list \
  "sendcloud" \
  "https://cdn.sendcloud.com/global-media/public-ips.txt"

imunify360-agent reload-lists

if [ "$FAILURES" -gt 0 ]; then
  echo "Sendcloud whitelist sync complete with $FAILURES failed source(s)."
  exit 1
fi

echo "Sendcloud whitelist sync complete."