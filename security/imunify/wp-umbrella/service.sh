...
TMP_DIR="$(mktemp -d)"
COMMENT_PREFIX="# managed-by=rubix-imunify-wp-umbrella"
...
fetch_plain_lists() {
  ...
  local tmp_raw="$TMP_DIR/${name}.raw"
  ...
  {
    echo "$COMMENT_PREFIX service=$name updated=$DATE source=plaintext"
    sort -u "$tmp_raw"
  } > "$tmp_out"
...
run_source "wp-umbrella" fetch_plain_lists \
  "wp-umbrella" \
  "https://wp-umbrella.com/ips-v4/" \
  "https://wp-umbrella.com/ips-monitoring/" \
  "https://wp-umbrella.com/ips-v6/"

imunify360-agent reload-lists

if [ "$FAILURES" -gt 0 ]; then
  echo "WP Umbrella whitelist sync complete with $FAILURES failed source(s)."
  exit 1
fi

echo "WP Umbrella whitelist sync complete."