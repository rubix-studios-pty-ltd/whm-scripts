...
TMP_DIR="$(mktemp -d)"
COMMENT_PREFIX="# managed-by=rubix-imunify-keycdn"
...
run_source "keycdn" fetch_json_array \
  "keycdn" \
...
if [ "$FAILURES" -gt 0 ]; then
  echo "KeyCDN whitelist sync complete with $FAILURES failed source(s)."
  exit 1
fi

echo "KeyCDN whitelist sync complete."