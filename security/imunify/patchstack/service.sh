...
TMP_DIR="$(mktemp -d)"
COMMENT_PREFIX="# managed-by=rubix-imunify-patchstack"
...
run_source "patchstack" fetch_plain_lists \
  "patchstack" \
  "https://patchstack.com/ips-v4/" \
  "https://patchstack.com/ips-v6/"

imunify360-agent reload-lists

if [ "$FAILURES" -gt 0 ]; then
  echo "Patchstack whitelist sync complete with $FAILURES failed source(s)."
  exit 1
fi

echo "Patchstack whitelist sync complete."