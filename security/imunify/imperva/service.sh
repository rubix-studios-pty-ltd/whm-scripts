...
TMP_DIR="$(mktemp -d)"
COMMENT_PREFIX="# managed-by=rubix-imunify-imperva"
...
run_source "imperva" fetch_imperva_json \
  "imperva" \
...
if [ "$FAILURES" -gt 0 ]; then
  echo "Imperva whitelist sync complete with $FAILURES failed source(s)."
  exit 1
fi

echo "Imperva whitelist sync complete."