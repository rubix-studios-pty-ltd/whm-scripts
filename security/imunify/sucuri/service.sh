...
TMP_DIR="$(mktemp -d)"
COMMENT_PREFIX="# managed-by=rubix-imunify-sucuri"
...
run_source "sucuri" fetch_static_prefixes \
  "sucuri" \
  "192.88.134.0/23" \
  "185.93.228.0/22" \
  "66.248.200.0/22" \
...
if [ "$FAILURES" -gt 0 ]; then
  echo "Sucuri whitelist sync complete with $FAILURES failed source(s)."
  exit 1
fi

echo "Sucuri whitelist sync complete."