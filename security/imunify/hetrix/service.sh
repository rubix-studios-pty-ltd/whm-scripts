...
TMP_DIR="$(mktemp -d)"
COMMENT_PREFIX="# managed-by=rubix-imunify-hetrix"
...
run_source "hetrix" fetch_plain_list \
  "hetrix" \
...
if [ "$FAILURES" -gt 0 ]; then
  echo "Hetrix whitelist sync complete with $FAILURES failed source(s)."
  exit 1
fi

echo "Hetrix whitelist sync complete."