...
TMP_DIR="$(mktemp -d)"
COMMENT_PREFIX="# managed-by=rubix-imunify-amazon-s3"
...
fetch_aws_s3_json() {
...

  {
    ...
    jq -r '
      (.prefixes[]? | select(.service == "S3") | .ip_prefix),
      (.ipv6_prefixes[]? | select(.service == "S3") | .ipv6_prefix)
    ' "$tmp_json" | sort -u
  } > "$tmp_out"
...
run_source "amazon-s3" fetch_aws_s3_json \
  "amazon-s3" \
...
if [ "$FAILURES" -gt 0 ]; then
  echo "Amazon S3 whitelist sync complete with $FAILURES failed source(s)."
  exit 1
fi

echo "Amazon S3 whitelist sync complete."